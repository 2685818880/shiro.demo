...
public class VinuxPostMethod extends PostMethod {
	 ...
	 * @param <T>
	 * @param requiredType
	 *            支持[net.sf.json.JSONArray,net.sf.json.JSONObject,String]
	 * @return
	 */
	...
	public Object executeMethod(String... type) {
		try {
			...
			/**
			 * 请求超时时间 如果需要调整，请setTimeOut(自定义时间(单位毫秒))
			 */
			// 加入额外的请求头信息
			...
			try {
				...
				LoggerUtils.error(getClass(), "请求[" + url + "]超时或错误,message : "
						+ e.getMessage(), e);
				RuntimeException ve = new RuntimeException("请求[" + url
						+ "]超时或错误,message : " + e.getMessage(), e);
				throw ve;
			}
			...
			if (status == HttpStatus.SC_OK) {
				...
				LoggerUtils.error(getClass(), "HTTP请求错误，请求地址为：[" + url
						+ "],状态为：[" + status + "]");
				throw new RuntimeException("HTTP请求错误，请求地址为：[" + url + "],状态为：["
						+ status + "]");
			}
