...
public class Ferrmarker {
	...
	static {
		...
		/** 添加自定义标签 */
		APITemplateModel api = SpringContextUtil.getBean("api",
				APITemplateModel.class);
		cfg.setSharedVariable("api", api);

		FreeMarkerConfigExtend ext = SpringContextUtil.getBean(
				"freemarkerConfig", FreeMarkerConfigExtend.class);
		...
		/**
		 * 获取配置文件里的 Configuration.settings 设到当前Cfg里 Properties setting = new
		 * Properties(); Map<String,String> sets = vcfg.getSettings();
		 * setting.putAll(sets); try { cfg.setSettings(setting); } catch
		 * (TemplateException e) { logger.error("添加settings error;" ,e); }
		 */
	}
	 ...
	 * @throws Exception
	 */
	public void outHtml(String path, String inFile, String outPath,
			String outFile, Map<String, Object> outMap) throws IOException,
			TemplateException {
		...
		try {
			...
			try {
				...
				logger.error("创建 [" + outFile + "] . io close exception"
						+ e.getMessage());
			}
		}
