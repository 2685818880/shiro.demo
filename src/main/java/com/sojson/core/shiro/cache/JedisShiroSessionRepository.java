...
public class JedisShiroSessionRepository implements ShiroSessionRepository {
	...
	public void saveSession(Session session) {
		...
		try {
			...
			if (null == session
				...
				session.setAttribute(CustomSessionManager.SESSION_STATUS,
						sessionStatus);
			}
			...
			LoggerUtils.fmtError(getClass(), e, "save session error，id:[%s]",
					session.getId());
		}
	}
	...
	public Collection<Session> getAllSessions() {
		...
		try {
			sessions = getJedisManager().AllSession(DB_INDEX,
					REDIS_SHIRO_SESSION);
		} catch (Exception e) {
