...
public class KickoutSessionFilter extends AccessControlFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request,
		...
		LinkedHashMap<Long, Serializable> infoMap = cache.get(ONLINE_USER,
				LinkedHashMap.class);
		// 如果不存在，创建一个新的
		infoMap = null == infoMap ? new LinkedHashMap<Long, Serializable>()
				: infoMap;
		...
		if (infoMap.containsKey(userId) && !infoMap.containsValue(sessionId)) {
			...
			if (null != oldSession) {
				...
				LoggerUtils.fmtDebug(getClass(),
						"kickout old session success,oldId[%s]", oldSessionId);
			} else {
