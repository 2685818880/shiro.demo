...
public class TokenManager {
	// 用户登录管理
	public static final SampleRealm realm = SpringContextUtil.getBean(
			"sampleRealm", SampleRealm.class);
	// 用户session管理
	 ...
	 * <code> doGetAuthorizationInfo(...)  </code>方法。 这里只是说明下这个逻辑，当你清空了权限，
	 * <code> doGetAuthorizationInfo(...)  </code>就会被再次调用。
	 */
	...
	public static void clearUserAuthByUserId(Long... userIds) {
		...
		if (null == userIds || userIds.length == 0)
			return;
		List<SimplePrincipalCollection> result = customSessionManager
				.getSimplePrincipalCollectionByUserId(userIds);
